		...
		case 'file':
			...
			await this.http
				...
				.then(res => {
					...
					const projectIds = res.data.projects.map(project => project.id);
					const projectFiles = projectIds.map(
						async projectId => this.http
							.get(
		...
		const response = context
			...
			.updateModalViewResponse({
				...
				close: block.newButtonElement({
					text: {
						type: TextObjectType.PLAINTEXT, text: 'Cancel'
					},
				}),
				submit: block.newButtonElement({
					actionId: 'secondModal',
					text: {
						type: TextObjectType.PLAINTEXT, text: 'Submit'
					},
				}),
