...
export async function commentEvent(room, read: IRead, roomData, user, payload: ICommentPayload, modify, comment, persistence) {
	...
	if (room) {
		...
		if (roomData.file_Ids || roomData.file_Ids!.length > 0) {
			...
			await getAllUsers(read).then((users) => {
				commentedBy = users.find((user) => user.figmaUserId === payload.triggered_by.id);
				...
				block.addActionsBlock({
					...
					elements: [
						block.newButtonElement({
							...
							value: {
								commentId: payload.comment_id, fileKey: payload.file_key
							},
						}),
						block.newButtonElement({
				...
				payload.comment.forEach(async (key: { mention: string }) => {
					if (key.mention) {
						...
						await getAllUsers(read)
							...
							.catch((e) =>
								console.log( 'error finding user in db mentioned in comment - ', e )
							);
						if (taggedUser) {
							await sendDMToUser(
								read,
